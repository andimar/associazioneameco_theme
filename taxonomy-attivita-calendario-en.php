...
<?php $biographies = new Biographies(); ?>
...
<div class="main row">
  ...
  <div class="large-12 medium-12 small-12 columns tabs-calendario-completo">
    ...
    <div class="text-format">
        ...
        $args = array(
          ...
          'tax_query' => array(
            array(
              'taxonomy' => 'attivita',
              'field' => 'slug',
              'terms' => array('ritiri-residenziali-en'),
              'operator' => 'NOT IN'
            )
          ),
          'meta_query' => array(
            'relation' => 'OR',
            array( 'key' => 'data_fine', 'compare' => '>=', 'value' => $current_date ),
            array( 'key' => 'data_inizio', 'compare' => '>=', 'value' => $current_date ),
          ),
      ...
      ?>
      <?php if($seminari_intensivi_query->have_posts()) : ?>
        <?php while($seminari_intensivi_query->have_posts()) : $seminari_intensivi_query->the_post(); ?>
          ...
          <?php if(is_user_logged_in() && current_user_can('edit_post')) : ?>
            <div style="color:#555;font-size:12px; margin-left:30px">
              ...
              <a href="/wp-admin/post.php?post=<?=$post->ID?>&action=edit">[modifica l'iniziativa]</a>
            </div>
          <?php endif;?>
          
          <a href="<?php the_permalink(); ?>">
            <h4 class="<?=$tipologia?>"><?php the_title(); ?></h4>
          </a>

          <span class="teacher-tag teacher-tag-<?=$tipologia?>">
            <small><?=pll_e('condotto da')?>:</small><br><?=$teacher?>
          </span>
          <span class="date-tag-<?=$tipologia?>"><?=$data_tag_testo?></span>
      ...
      ?>
      <?php if($ritiri_query->have_posts()) : ?>
